...
func (r *InputRecord) GetTime() time.Time {
	if r.isProcessable {
		if r.Timestamp[len(r.Timestamp)-1] == 'Z' {
			return conversion.ConvertDatetimeStringWithMillisNoTZ(r.Timestamp[:len(r.Timestamp)-1] + "000")
		}
		return conversion.ConvertDatetimeStringWithMillisNoTZ(r.Timestamp + "000")
	}
	return time.Time{}
}

...
func (t *Transformer) Transform(logRecord *InputRecord, recType string, anonymousUsers []int) (*OutputRecord, error) {
	...
	isRegexp, err := importBool(logRecord.IsRegexp, "isRegexp")
	if err != nil {
		return nil, err
	}
	isCaseInsen, err := importBool(logRecord.IsCaseInsen, "isCaseInsen")
	if err != nil {
		return nil, err
	}
	isMultiWord, err := importBool(logRecord.IsMultiWord, "isMultiWord")
	if err != nil {
		return nil, err
	}
	isLemma, err := importBool(logRecord.IsMultiWord, "isLemma")
	if err != nil {
		return nil, err
	}
